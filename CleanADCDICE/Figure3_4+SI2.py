...
plt.tight_layout()
ax2 = plot_prctiles(output, 'CO2 Ind. Emissions [GtCO2]', color='forestgreen')
...
for filt in filters:
       ...
       for cat in cats:
              ...
              ax = plot_prctiles(sel, 'CO2 Ind. Emissions [GtCO2]', ax=ax, color=colors[count], label=cat)
              count += 1       
       ax.legend()
...
plt.tight_layout()
ax2 = plot_prctiles(output, 'CO2 Ind. Emissions [GtCO2]', ax=ax2, color='red')
plt.tight_layout()  
